...
static void		ConvertToInterpolationCoefficients
    ...
    long    BoundariesType  /* 0 for mirror, 1 for periodic */
	);
...
static double	InitialCausalCoefficientPeriodicBoundaries
	(
		double	c[],		/* coefficients */
		long	DataLength,	/* number of coefficients */
		double	z,			/* actual pole */
...
static double	InitialAntiCausalCoefficientPeriodicBoundaries
	(
		double	c[],		/* coefficients */
		long	DataLength,	/* number of samples or coefficients */
		double	z,			/* actual pole */
...
{ /* begin ConvertToInterpolationCoefficients */
  ...
  /* loop over all poles */
	  for (k = 0L; k < NbPoles; k++) {
      /* causal initialization */
      c[0] = InitialCausalCoefficientPeriodicBoundaries(c, DataLength, z[k], Tolerance);
      /* causal recursion */
      for (n = 1L; n < DataLength; n++) {
        c[n] += z[k] * c[n - 1L];
      }
      /* anticausal initialization */
      c[DataLength - 1L] = InitialAntiCausalCoefficientPeriodicBoundaries(c, DataLength, z[k], Tolerance);
	    /* anticausal recursion */
      for (n = DataLength - 2L; 0 <= n; n--) {
        c[n] = z[k] * (c[n + 1L] - c[n]);
      }
    ...
    fprintf(stderr,"Unknown boundaries type. Allowed are mirror or periodic \n");
    exit(1);
  }
...
{ /* begin InitialCausalCoefficientPeriodicBoundaries */
...
} /* end InitialCausalCoefficientPeriodicBoundaries */
...
{ /* begin InitialAntiCausalCoefficientPeriodicBoundaries */
  /* No smart closed-form in this case, because no mirror condition */
...
} /* end InitialAntiCausalCoefficientPeriodicBoundaries */
...
{ /* begin SamplesToCoefficients */
	...
	//double	*Line;
	// Static allocation to make it private in openmp loop
	double  Line[LINE_SIZE_MAX];
	double	Pole[4];
	long	NbPoles;
	long	x, y;
	...
	*/
#pragma omp parallel default (shared) private (y,Line) 
{
#pragma omp for schedule (static)
	for (y = 0L; y < Height; y++) {
		GetRow(Image, y, Line, Width);
		ConvertToInterpolationCoefficients(Line, Width, Pole, NbPoles, DBL_EPSILON, BoundariesType);
	...
	*/
#pragma omp parallel default (shared) private (x,Line)
{
#pragma omp for schedule (static)
	for (x = 0L; x < Width; x++) {
		GetColumn(Image, Width, x, Line, Height);
		ConvertToInterpolationCoefficients(Line, Height, Pole, NbPoles, DBL_EPSILON, BoundariesType);
...
/*--------------------------------------------------------------------------*/
/* 1D coefficients computation routine */
/*--------------------------------------------------------------------------*/


extern int		SamplesToCoefficients1D
	(
		double	*Data,		/* in-place processing */
		long   	Datasize,	/* length of Data */
		long		SplineDegree,   /* degree of the spline model */
		long    BoundariesType  /* mirror or periodic */
	)

{ /* begin SamplesToCoefficients1D */
  ...
  long i;
	...
	/* convert the Data samples into interpolation coefficients */
        ConvertToInterpolationCoefficients(Data, Datasize, Pole, NbPoles, DBL_EPSILON, BoundariesType);

	return(0);
} /* end SamplesToCoefficients1D */
